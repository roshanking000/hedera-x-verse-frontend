...
import MakeOffer from '../../common/loan/MakeOffer'
...
const LendAssetDetailsComponent = () => {
    const params = useParams()

    const { walletData, allowanceHbar } = useHashConnect();
    ...
    const getCollateralInfo = async () => {
        setLoadingView(true);
        // get collateral info
        const _res = await getRequest(env.SERVER_URL + "/api/collateral/get_item_detail?id=" + params.id);
        if (!_res) {
        ...
        setCollateralInfo(_res.nftInfo);
        setLoanAmount(_res.nftInfo.loan_amount);
        setBorrowerFee(_res.nftInfo.borrower_fee);
        setMaturityDate(_res.nftInfo.maturity_date);

        setOfferList(_res.offerList);
        setLoadingView(false);
    }

    const onClickSendOffer = async (loanAmount_, borrowerFee_, maturityDate_) => {
        setLoadingView(true);

        const _trxResult = await allowanceHbar(loanAmount_);
        ...
        const _postData = {
            sender: accountIds[0],
            offerType: 'lend',
            collateralId: collateralInfo._id,
            loanAmount: loanAmount_,
            borrowerFee: borrowerFee_,
            maturityDate: maturityDate_
        };

        const _res = await postRequest(env.SERVER_URL + "/api/collateral/send_offer", _postData);
        if (!_res) {
            toast.error("Something wrong with server!");
            setLoadingView(false);
            return;
    ...
    const onClickCancelListing = async () => {
        ...
        // cancel listing
        const _postData = {
            token_id: collateralInfo.token_id,
            serial_number: collateralInfo.serial_number
        };
            ...
            <div className='h-72  bg-navy w-full flex justify-center'>
                <div className='flex flex-col w-5/6 justify-between'>
                    <div className='page-header flex flex-col gap-y-2 py-5'>
                        <div className='heading font-semibold text-7xl font-Lato'>{collateralInfo?.name}</div>
                        <div className='sub-heading text-xl font-Roboto'>{collateralInfo?.creator}</div>
                    </div>
                </div>
            </div>
                        ...
                        <div className='panel image-panel relative flex flex-col gap-y-2 bg-navy_medium p-5'>
                            ...
                            <div className='image-preview h-full'>
                                ...
                                <video
                                    ...
                                    loop
                                >
                                    <source src={collateralInfo?.imageUrl} />
                                </video>
                            </div>
                        </div>
                        <div className='bg-navy_medium p-5'>
                            <div className='asset-info-panel lend panel info-panel h-full flex flex-col'>
                                <div className='header flex flex-col gap-y-1'>
                                    ...
                                    <h2 className='text-2xl font-bold'>{collateralInfo?.name}</h2>
                                    <h4>
                                        <a
                                        ...
                                        <div className='py-2'>
                                            <div className='group grid grid-cols-2 gap-4'>
                                                <div className='item'>
                                                    ...
                                                    <p className='text-xl font-Lato'>n/a</p>
                                                </div>
                                                <div className='item'>
                                                    <h3 className='flex items-center py-1 gap-x-3'>
                                                        <svg
                                        ...
                                        showMakeOffer === false ? (
                                            <div className='step summary flex flex-col h-full flex-1 gap-y-2'>
                                                ...
                                                <div className='buttons'>
                                                    {
                                                        accountIds && accountIds[0] !== collateralInfo?.owner &&
                                                        <button
                                            ...
                                            <MakeOffer
                                                onClickSendOffer={(loanAmount_, borrowerFee_, maturityDate_) => {
                                                    onClickSendOffer(loanAmount_, borrowerFee_, maturityDate_);
                                                }}
                                            />
                                        )
...
export default LendAssetDetailsComponent
