              ...
              previousLoanList?.map((item, index) => {
                return (
                  ...
                  >
                    ...
                    <div className='sortable flex items-center w-full justify-center'>{item.status}</div>
                  </Link>
                )
              })}
          </div>
...
const BorrowLoanComponent = () => {
  ...
  const getCurrentLoanList = async () => {
    const _res = await getRequest(env.SERVER_URL + "/api/loan/get_current_loan_list?accountId=" + accountIds[0] + "&user_type=" + "borrow");
    if (!_res) {
  ...
  const getPreviousLoanList = async () => {
    const _res = await getRequest(env.SERVER_URL + "/api/loanhistory/get_loanhistory_list?accountId=" + accountIds[0] + "&user_type=" + "borrow");
    if (!_res) {
      toast.error("Something wrong with server!");
      setLoadingView(false);
      return;
  ...
  const onClickRepay = async (id_) => {
    setLoadingView(true);
    const _res = await getRequest(env.SERVER_URL + "/api/loan/get_repay_amount?a=" + btoa(id_));
    if (!_res) {
      toast.error("Something wrong with server!");
      setLoadingView(false);
      return;
    ...
    const _repayAmount = _res.data;
    console.log(_repayAmount);
    const _trxResult = await allowanceHbar(_repayAmount);
    if (!_trxResult) {
    ...
    // go to the backend
    const _repayResult = await getRequest(env.SERVER_URL + "/api/loan/repay?a=" + btoa(id_));
    if (!_repayResult) {
      toast.error("Something wrong with server!");
      setLoadingView(false);
      return;
    }
    if (!_repayResult.result) {
      toast.error(_repayResult.error);
      setLoadingView(false);
      return;
    }
    ...
    toast.success(_repayResult.msg);
    setLoadingView(false);
  }
  ...
  return (
    <section className='flex flex-col gap-5 py-10'>
      <CurrentLoanAsBorrower
        ...
        onClickRepay={async (id_) => {
          await onClickRepay(id_);
        }}
      />
      <PreviousLoanAsBorrower previousLoanList={previousLoanList} />
      {
        loadingView &&
        <LoadingLayout />
      }
