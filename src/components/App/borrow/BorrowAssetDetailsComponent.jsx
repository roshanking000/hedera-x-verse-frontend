...
import ReceivedOffersList from '../lend/ReceivedOffersList'
import ProjectAverages from '../lend/ProjectAverages'
import MakeAggrement from '../../common/loan/MakeAggrement'
...
const BorrowAssetDetailsComponent = () => {
    const params = useParams()
    const navigate = useNavigate();

    const { walletData, sendNftToTreasury } = useHashConnect();
    const { accountIds } = walletData;

    const [loadingView, setLoadingView] = useState(false);
    const [showMakeAgreement, setShowMakeAgreement] = useState(false);

    const [nftInfo, setNftInfo] = useState(null);
    const [listState, setListState] = useState(false);
    const [offerList, setOfferList] = useState(null);
    const [zuseFloorPrice, setZuseFloorPrice] = useState(null);

    //offer
    const [loanAmount, setLoanAmount] = useState(0);
    const [borrowerFee, setBorrowerFee] = useState(1);
    const [maturityDate, setMaturityDate] = useState(1);
    ...
    useEffect(() => {
        if (listState.status == true)
            getOfferList(listState.id);
    }, [listState]);

    const getNftInfo = async () => {
        setLoadingView(true);
        await getNftInfoFromMirrorNet(params.token_id, params.serial_number);
        await getFloorPrice();
        await checkList(params.token_id, params.serial_number);
    ...
    const getFloorPrice = async () => {
        const _res = await getRequest(env.SERVER_URL + "/api/zuseapi/get_floor_price?token_id=" + params.token_id);
        console.log(_res);
        if (!_res) {
            toast.error("Something wrong with server!");
            setZuseFloorPrice("n/a");
            setLoadingView(false);
            return false;
        }
        if (!_res.result) {
    ...
    const checkList = async (tokenId_, serialNum_) => {
        const _res = await getRequest(env.SERVER_URL + "/api/collateral/check_list_state?token_id=" + tokenId_ + "&serial_number=" + serialNum_);
        if (!_res) {
            toast.error("Something wrong with server!");
            setLoadingView(false);
            return false;
        ...
        setLoanAmount(_res.data.loanAmount);
        setBorrowerFee(_res.data.borrowerFee);
        setMaturityDate(_res.data.maturityDate);
        return true;
    }

    // get offer list
    const getOfferList = async (id) => {
        const _res = await getRequest(env.SERVER_URL + "/api/collateral/get_offer_list?id=" + id);
        if (!_res) {
            toast.error("Something wrong with server!");
            setLoadingView(false);
            return;
    ...
    //list nft
    const onClickListNft = async (loanAmount_, borrowerFee_, maturityDate_) => {
        setLoadingView(true);
        ...
        const _postData = {
            ...
            serial_number: nftInfo.serial_number,
            loan_amount: loanAmount_,
            borrower_fee: borrowerFee_,
            maturity_date: maturityDate_,
            name: nftInfo.name,
            creator: nftInfo.creator,
            imageUrl: nftInfo.imageUrl
        };

        const _nftListRes = await postRequest(env.SERVER_URL + "/api/collateral/set_list", _postData);
        if (!_nftListRes) {
            toast.error("Something wrong with server!");
            setLoadingView(false);
            return;
        ...
        const _listStatus = await checkList(nftInfo.token_id, nftInfo.serial_number);
        if (_listStatus) {
            // success
    ...
    const onClickCancelListing = async () => {
        ...
        const _cancelListingRes = await postRequest(env.SERVER_URL + "/api/collateral/cancel_list", _postData);
        ...
        if (_listStatus) {
            // success
            toast.success(_cancelListingRes.msg);
            setLoadingView(false);
        }
    }
        ...
        <section className='w-full flex flex-col justify-center items-center lend-page h-full'>
            <div className='h-72  bg-navy w-full flex justify-center'>
                <div className='flex flex-col w-5/6 justify-between'>
                    <div className='page-header flex flex-col gap-y-2 py-5'>
                        <div className='heading font-semibold text-7xl font-Lato'>{nftInfo?.name}</div>
                        <div className='sub-heading text-xl font-Roboto'>{nftInfo?.creator}</div>
                    </div>
                </div>
            </div>
            <div className='w-2/3 flex flex-col min-h-[800px] py-5 mb-28'>
                ...
                <div className='py-5'>
                    <div className='grid grid-cols-1 lg:grid-cols-3 gap-3'>
                        <div className='panel image-panel relative flex flex-col gap-y-2 bg-navy_medium p-5'>
                            <div className='buttons-row flex items-center justify-end w-full gap-x-3 hidden'>
                        ...
                        <div className='bg-navy_medium p-5'>
                            <div className='asset-info-panel lend panel info-panel h-full flex flex-col'>
                                <div className='header flex flex-col gap-y-1'>
                                    ...
                                    <h2 className='text-2xl font-bold'>{nftInfo?.name}</h2>
                                    <h4>
                                        <a
                                    ...
                                    <p className='bg-primary text-white max-w-14 uppercase w-14 text-xs text-center px-1 py-0.5 rounded-full hidden'>
                                        listed
                                    </p>
                                    <div></div>
                                </div>
                                    ...
                                    <div className='price-information py-2'>
                                        ...
                                        <div className='py-2'>
                                            <div className='group grid grid-cols-2 gap-4'>
                                                <div className='item'>
                                                    <h2 className='flex items-center py-1 gap-x-3'>
                                                        <img width='24' height='24' loading='lazy' src='/icons/zuse-icon.png' />
                                                        Floor price
                                                    </h2>
                                                    <p className='text-xl font-Lato'>{zuseFloorPrice} ℏ</p>
                                                </div>
                        ...
                        <div className='bg-[#5c113e] p-5'>
                            ...
                            >
                                <div className='title font-semibold font-Lato'>Get a loan</div>
                                <div className='steps step-0 flex h-full'>
                                    {
                                        listState.status === false ? (
                                            showMakeAgreement === false ? (
                                                <div className='step summary flex flex-col h-full flex-1 gap-y-2'>
                                                    <div className='sub-title font-Roboto text-gray-300'>Loan agreement:</div>
                                                    <div className='sub-title max-w-max'>Loan Amount: Better if you stay under 50% LTV Based on last sale or FP</div>
                                                    <div className='sub-title max-w-max'>Borrower fee: 1%-20% (Displayed in APR)</div>
                                                    <div className='sub-title max-w-max'>Maturity date: 1-60 Days</div>
                                                    <div className='buttons'>
                                                        <button
                                                            id='make-offer-button-1 '
                                                            className='btn w-full py-2 px-5 text-center bg-secondary text-white font-Lato'
                                                            onClick={() => {
                                                                setShowMakeAgreement(true);
                                                            }}
                                                        >
                                                            Make loan agreement
                                                        </button>
                                                ...
                                                <MakeAggrement
                                                    onClickListNft={(loanAmount_, borrowerFee_, maturityDate_) => {
                                                        onClickListNft(loanAmount_, borrowerFee_, maturityDate_);
                                                    }}
                                                />
                                            )
                                            ...
                                            <div className='step summary flex flex-col h-full flex-1 gap-y-2'>
                                                ...
                                                <div className='terms flex flex-col gap-y-2 flex-1 h-full'>
                                                    <div className='term accent-color-background bg-secondary py-1 px-3 rounded-full max-w-max'>
                                                        <span>
                                                            For <strong>{loanAmount} ℏ</strong>
                                                        </span>
                                                    </div>
                                                    <div className='term accent-color-background bg-secondary py-1 px-3 rounded-full max-w-max'>
                                                        <span>
                                                            Over <strong>{maturityDate} days</strong>
                                                        </span>
                                                    </div>
                                                    <div className='term accent-color-background bg-secondary py-1 px-3 rounded-full max-w-max'>
                                                        {borrowerFee} APR
                ...
                <div className='py-2'>
                    <ReceivedOffersList
                        offerList={offerList}
                        onClickAcceptOffer={async (offerId_) => {
                            setLoadingView(true);
                            const _transactionResult = await sendNftToTreasury(params.token_id, params.serial_number);
                            ...
                            const _res = await postRequest(env.SERVER_URL + "/api/collateral/accept_offer", _postData);
                            if (!_res) {
                                toast.error("Something wrong with server!");
                                setLoadingView(false);
                                return;
...
export default BorrowAssetDetailsComponent
